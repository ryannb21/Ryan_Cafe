...
secret_key = get_secret(os.getenv("FLASK_CAFE_SECRET_NAME", "ryan-cafev31/flask_secret"), region_name=os.getenv("AWS_REGION", "us-east-1"))
...
email_secrets = get_secret(os.getenv("EMAIL_SECRET_NAME", "ryan-cafev31/emailcreds"), region_name=os.getenv("AWS_REGION", "us-east-1"))
...
db_secrets = get_secret(os.getenv("DB_SECRET_NAME", "ryan-cafev31/db_creds"), region_name=os.getenv("AWS_REGION", "us-east-1"))
...
#Configuring readiness helper for /health
...
def initialize_database():
    try:
        #Connecting to MySQL without specifying database first
...
#Defining an authoritatize prize map
...
def place_order():
    ...
    try:
        for val in items:
            ...
            #Implementing the authoritatize price look-up to avoid price tampering
            if name not in MENU:
